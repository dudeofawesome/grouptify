...
@Component({
    selector: "project-swipe-card",
    templateUrl: "build/components/project-swipe-card/project-swipe-card.html",
    directives: [IONIC_DIRECTIVES, Chip],
    pipes: [],
    providers: [],
    inputs: ["node"]
})
export class ProjectSwipeCard {
    @Input() project: any;
    el: ElementRef;
    ...
    @Output() swiped: EventEmitter<{id: string; dir: string}> = new EventEmitter();
    @Output() offscreen: EventEmitter<string> = new EventEmitter();
    ...
    dragStop (ev: TouchEvent) {
        if (Math.abs(this.position.x) > this.el.nativeElement.firstChild.offsetWidth / 2) {
            ...
            this.swiped.emit({id: this.project._id, dir: this.position.x > 0 ? "right" : "left"});
